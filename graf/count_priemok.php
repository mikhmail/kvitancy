...
// Подключение к БД.
$db1 = mysql_connect('localhost', 'root', '', 'kvitancy');
					mysql_select_db('kvitancy', $db1);
					mysql_query("SET NAMES 'utf8'", $db1);
...
//$arr_month[] = '0'.$i;
	...
	if($i != $id_month ) {
		$where1 .= "(SELECT COUNT(*) FROM kvitancy WHERE date_priemka between '$id_year-0$i-01' and '$id_year-0$i-31' AND id_sc = 1) AS cnt_$i,";
	} else {
		$where1 .= "(SELECT COUNT(*) FROM kvitancy WHERE date_priemka between '$id_year-0$i-01' and '$id_year-0$i-31' AND id_sc = 1) AS cnt_$i";
	}
}
...
for ($i=1; $i<=$id_month; $i++) {
	...
	if($i != $id_month ) {
		$where2 .= "(SELECT COUNT(*) FROM kvitancy WHERE date_priemka between '$id_year-0$i-01' and '$id_year-0$i-31' AND id_sc = 13) AS cnt_$i,";
	} else {
		$where2 .= "(SELECT COUNT(*) FROM kvitancy WHERE date_priemka between '$id_year-0$i-01' and '$id_year-0$i-31' AND id_sc = 13) AS cnt_$i";
	}
}
...
for ($i=1; $i<=$id_month; $i++) {
	...
	if($i != $id_month ) {
		$where3 .= "(SELECT COUNT(*) FROM kvitancy WHERE date_priemka between '$id_year-0$i-01' and '$id_year-0$i-31' AND id_sc = 14) AS cnt_$i,";
	} else {
		$where3 .= "(SELECT COUNT(*) FROM kvitancy WHERE date_priemka between '$id_year-0$i-01' and '$id_year-0$i-31' AND id_sc = 14) AS cnt_$i";
	}
}
