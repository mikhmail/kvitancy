    ...
    public function index()
    {
        ...
        if($search_string !== false && $order !== false || $this->uri->segment(3) == true){ 
            ...
            */
            ...
            $data['count_products']= $this->sost_remonta_model->count_sost_remonta($search_string, $order);
            $config['total_rows'] = $data['count_products'];

            //fetch sql data into arrays
            if($search_string){
                if($order){
                    $data['manufacturers'] = $this->sost_remonta_model->get_sost_remonta($search_string, $order, $order_type, $config['per_page'],$limit_end);        
                }else{
                    $data['manufacturers'] = $this->sost_remonta_model->get_sost_remonta($search_string, '', $order_type, $config['per_page'],$limit_end);           
                }
            }else{
                if($order){
                    $data['manufacturers'] = $this->sost_remonta_model->get_sost_remonta('', $order, $order_type, $config['per_page'],$limit_end);        
                }else{
                    $data['manufacturers'] = $this->sost_remonta_model->get_sost_remonta('', '', $order_type, $config['per_page'],$limit_end);        
                }
            }
            ...
            //fetch sql data into arrays
            $data['count_products']= $this->sost_remonta_model->count_sost_remonta();
            $data['manufacturers'] = $this->sost_remonta_model->get_sost_remonta('', '', $order_type, $config['per_page'],$limit_end);        
            $config['total_rows'] = $data['count_products'];
