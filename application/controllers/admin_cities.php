    ...
    */
    public function index()
    {
        ...
        if($search_string !== false && $order !== false || $this->uri->segment(3) == true){ 
            ...
            */
            ...
            $data['count_products']= $this->gorod_model->count_gorod($search_string, $order);
            $config['total_rows'] = $data['count_products'];

            //fetch sql data into arrays
            if($search_string){
                if($order){
                    $data['manufacturers'] = $this->gorod_model->get_gorod($search_string, $order, $order_type, $config['per_page'],$limit_end);        
                }else{
                    $data['manufacturers'] = $this->gorod_model->get_gorod($search_string, '', $order_type, $config['per_page'],$limit_end);           
                }
            }else{
                if($order){
                    $data['manufacturers'] = $this->gorod_model->get_gorod('', $order, $order_type, $config['per_page'],$limit_end);        
                }else{
                    $data['manufacturers'] = $this->gorod_model->get_gorod('', '', $order_type, $config['per_page'],$limit_end);        
                }
            }
            ...
            //fetch sql data into arrays
            $data['count_products']= $this->gorod_model->count_gorod();
            $data['manufacturers'] = $this->gorod_model->get_gorod('', '', $order_type, $config['per_page'],$limit_end);        
            $config['total_rows'] = $data['count_products'];
    ...
    public function add()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            {
                $data_to_store = array(
                    'gorod' => $this->input->post('name'),
                );
                //if the insert has returned true then we show the flash message
                if($this->gorod_model->store_gorod($data_to_store)){
    ...
    public function update()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            if ($this->form_validation->run())
            {
                ...
                //if the insert has returned true then we show the flash message
                if($this->gorod_model->update_gorod($id, $data_to_store) == TRUE){
